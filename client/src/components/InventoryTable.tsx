...
export type InventoryProduct = { id: number; name: string; sku: string; barcode?: string | null; unit?: string; salePrice: string; stockQty: number };
...
export function InventoryTable({ products, search, loading = false, error, onSearch, onEdit }: { products: InventoryProduct[]; search: string; loading?: boolean; error?: unknown; onSearch: (value: string) => void; onEdit: (product: InventoryProduct) => void }) {
  ...
  return <section className="border-0 bg-white shadow-[0_12px_35px_rgba(15,23,42,0.06)]">
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-right"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-4">اسم المنتج</th><th className="p-4">الكود</th><th className="p-4">الوحدة</th><th className="p-4">العدد</th><th className="p-4">السعر</th><th className="p-4">إجراء</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map(product => <tr key={product.id} className="text-sm"><td className="p-4 font-bold">{product.name}<span className="block text-xs text-slate-400 mt-1">{product.barcode || "بدون باركود"}</span></td><td className="p-4 font-mono text-xs" dir="ltr">{product.sku}</td><td className="p-4">{product.unit || "قطعة"}</td><td className="p-4 font-bold">{product.stockQty}</td><td className="p-4 font-bold">{Number(product.salePrice).toLocaleString("ar-EG")} ج.م</td><td className="p-4"><Button variant="outline" onClick={() => onEdit(product)} className="rounded-none gap-2"><Pencil size={14}/> تعديل</Button></td></tr>)}{!filtered.length && <tr><td colSpan={6} className="p-12 text-center text-sm text-slate-400">لا توجد منتجات مطابقة للبحث.</td></tr>}</tbody></table></div>
  </section>;
}
