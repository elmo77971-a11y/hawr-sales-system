...
function LocalLogin({ onSuccess }: { onSuccess: () => Promise<void> }) {
  ...
  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    setError("");
    login.mutate({ username, password }, { onSuccess: () => void onSuccess(), onError: e => setError(e.message) });
  };
  return <AuthCard title="تسجيل الدخول" description="اكتب اسم المستخدم وكلمة المرور للدخول إلى النظام المحلي."><form onSubmit={submit} className="space-y-4"><label className="block text-sm font-bold">اسم المستخدم<Input required minLength={1} value={username} onChange={e => setUsername(e.target.value)} placeholder="username" dir="ltr" className="mt-2 h-11 rounded-none text-left" /></label><label className="block text-sm font-bold">كلمة المرور<Input required minLength={1} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="كلمة المرور" dir="ltr" className="mt-2 h-11 rounded-none text-left" /></label>{error && <p className="bg-red-50 p-3 text-sm font-bold text-red-700">{error}</p>}<Button type="submit" disabled={pending} className="h-11 w-full rounded-none bg-slate-950">{pending ? "جارٍ التحقق..." : "دخول إلى النظام"}</Button></form></AuthCard>;
}
