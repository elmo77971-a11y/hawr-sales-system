...
function LocalLogin({ onSuccess }: { onSuccess: () => Promise<void> }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (mode === "manager") {
      ...
      employeeLogin.mutate({ name, employeeCode, password }, { onSuccess: () => void onSuccess(), onError: e => setError(e.message) });
    }
  };
  return <AuthCard title="تسجيل الدخول" description="اكتب اسم المستخدم وكلمة المرور للدخول إلى النظام المحلي."><div className="mb-6 grid grid-cols-2 gap-3 text-xs font-bold"><button type="button" onClick={() => { setMode("employee"); setError(""); }} className={mode === "employee" ? "flex items-center gap-2 bg-slate-950 p-3 text-white" : "flex items-center gap-2 bg-slate-100 p-3 text-slate-600"}>دخول موظف</button><button type="button" onClick={() => { setMode("manager"); setError(""); }} className={mode === "manager" ? "flex items-center gap-2 bg-red-600 p-3 text-white" : "flex items-center gap-2 bg-red-50 p-3 text-red-700"}><ShieldCheck size={16} /> دخول مدير</button></div><form onSubmit={submit} className="space-y-4"><label className="block text-sm font-bold">الاسم الكامل<Input required minLength={2} value={name} onChange={e => setName(e.target.value)} placeholder={mode === "manager" ? "اسم المدير" : "اسم الموظف"} className="mt-2 h-11 rounded-none" /></label><label className="block text-sm font-bold">اسم المستخدم / الكود<Input required minLength={1} value={employeeCode} onChange={e => setEmployeeCode(e.target.value)} placeholder={mode === "manager" ? "MANAGER01" : "EMP001"} dir="ltr" className="mt-2 h-11 rounded-none text-left" /></label><label className="block text-sm font-bold">كلمة المرور<Input required minLength={1} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="كلمة المرور" dir="ltr" className="mt-2 h-11 rounded-none text-left" /></label>{error && <p className="bg-red-50 p-3 text-sm font-bold text-red-700">{error}</p>}<Button type="submit" disabled={pending} className="h-11 w-full rounded-none bg-slate-950">{pending ? "جارٍ التحقق..." : "دخول إلى النظام"}</Button></form></AuthCard>;
}
