...
import { exportReportToExcel, exportReportToPdf, type ReportRow } from "@/lib/exports";
import { InvoiceForm } from "@/components/InvoiceForm";
import { InventoryTable, type InventoryProduct } from "@/components/InventoryTable";
import { printLastReceipt } from "@/lib/receipt";
...
export default function Home() {
  ...
  const displayProducts = remoteProducts?.length ? remoteProducts.map(p => ({ name: p.name, sku: p.sku, barcode: p.barcode, unit: p.unit, category: "منتجات", stock: p.stockQty, price: `${p.salePrice} ج.م`, tone: "bg-red-50 text-red-700" })) : productSeed.map(p => ({ ...p, barcode: "", unit: "قطعة" }));
  ...
  return <div dir="rtl" className="min-h-screen bg-[#f7f7f5] text-slate-950 font-sans">
    ...
    <main className="lg:mr-[264px] min-h-screen">
      ...
      <div className="p-5 md:p-9 max-w-[1500px] mx-auto">
        ...
        </> : <ModulePage module={active} label={currentLabel} summary={remoteSummary} firstProduct={remoteProducts?.[0]} products={remoteProducts || []} productsLoading={productsLoading} productsError={productsError} onBack={() => setActive("dashboard")} />}
      </div>
    </main>
  </div>;
}
...
function ModulePage({ module, label, summary, firstProduct, products, productsLoading, productsError, onBack }: { module: NavKey; label: string; summary?: { sales: number; profit: number; stockValue: number; installmentsDue: number } | null; firstProduct?: { id: number; salePrice: string }; products: { id: number; name: string; sku: string; barcode?: string | null; unit?: string; salePrice: string; costPrice?: string | null; stockQty: number }[]; productsLoading?: boolean; productsError?: unknown; onBack: () => void }) {
  ...
  const [productDraft, setProductDraft] = useState({ id: 0, name: "", sku: "", unit: "قطعة", stockQty: "0", salePrice: "0", barcode: "" });
  ...
  const saveProduct = () => { const data = { name: productDraft.name.trim(), sku: productDraft.sku.trim() || `SKU-${Date.now()}`, barcode: productDraft.barcode.trim() || undefined, unit: productDraft.unit.trim() || "قطعة", stockQty: Math.max(0, Number(productDraft.stockQty) || 0), salePrice: productDraft.salePrice || "0" }; if (!data.name) return; const done = () => { void trpcUtils.catalog.products.invalidate(); setDrawerOpen(false); setProductDraft({ id: 0, name: "", sku: "", unit: "قطعة", stockQty: "0", salePrice: "0", barcode: "" }); }; if (productDraft.id) productUpdate.mutate({ id: productDraft.id, ...data }, { onSuccess: done }); else productCreate.mutate(data, { onSuccess: done }); };
  const startProductEdit = (product?: { id: number; name: string; sku: string; unit?: string; stockQty: number; salePrice: string; barcode?: string | null }) => { setProductDraft(product ? { id: product.id, name: product.name, sku: product.sku, unit: product.unit || "قطعة", stockQty: String(product.stockQty), salePrice: product.salePrice, barcode: product.barcode || "" } : { id: 0, name: "", sku: "", unit: "قطعة", stockQty: "0", salePrice: "0", barcode: "" }); setDrawerOpen(true); };
  const save = () => {
