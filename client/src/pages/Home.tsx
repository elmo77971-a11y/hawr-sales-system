...
export default function Home() {
  ...
  const filteredProducts = useMemo(() => products.filter(p => p.name.includes(search) || p.sku.toLowerCase().includes(search.toLowerCase())), [search]);
  ...
  return <div dir="rtl" className="min-h-screen bg-[#f7f7f5] text-slate-950 font-sans">
    ...
    <main className="lg:mr-[264px] min-h-screen">
      ...
      <div className="p-5 md:p-9 max-w-[1500px] mx-auto">
        ...
        </> : <ModulePage module={active} label={currentLabel} onBack={() => setActive("dashboard")} />}
      </div>
    </main>
  </div>;
}
...
function ModulePage({ module, label, onBack }: { module: NavKey; label: string; onBack: () => void }) {
  const copy = moduleCopy[module];
  const [rows, setRows] = useState<{ id: number; title: string; detail: string; value: string }[]>(() => {
    try { return JSON.parse(localStorage.getItem(`arousain-${module}`) || "[]"); } catch { return []; }
  });
  ...
  const save = () => {
    ...
    const next = [{ id: Date.now(), title: title.trim() || "تقرير جديد", detail: detail.trim() || "تم الحفظ محليًا بانتظار المزامنة", value: detail.trim() || "—" }, ...rows];
    setRows(next); localStorage.setItem(`arousain-${module}`, JSON.stringify(next)); enqueueOperation(module, { title: title.trim() || "تقرير جديد", detail: detail.trim() }); setTitle(""); setDetail(""); setDrawerOpen(false);
  };
  return <div>
