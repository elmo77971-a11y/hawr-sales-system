  ...
  return <div dir="rtl" className="min-h-screen bg-[#f7f7f5] text-slate-950 font-sans">
    ...
    <main className="lg:mr-[264px] min-h-screen">
      ...
      <div className="p-5 md:p-9 max-w-[1500px] mx-auto">
        ...
        </> : <ModulePage module={active} label={currentLabel} summary={remoteSummary} firstProduct={remoteProducts?.[0]} products={remoteProducts || []} onBack={() => setActive("dashboard")} />}
      </div>
    </main>
  </div>;
}
...
function ModulePage({ module, label, summary, firstProduct, products, onBack }: { module: NavKey; label: string; summary?: { sales: number; profit: number; stockValue: number; installmentsDue: number } | null; firstProduct?: { id: number; salePrice: string }; products: { id: number; name: string; sku: string; barcode?: string | null; salePrice: string; stockQty: number }[]; onBack: () => void }) {
