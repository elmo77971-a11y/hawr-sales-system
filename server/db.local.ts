...
function getLocalDb() {
  ...
  localDb.exec(`
    ...
    CREATE TABLE IF NOT EXISTS sales (
      id INTEGER PRIMARY KEY AUTOINCREMENT, invoiceNo TEXT NOT NULL UNIQUE, customerId INTEGER, customerName TEXT, customerPhone TEXT, sellerId INTEGER, sellerCode TEXT,
    ...
    CREATE TABLE IF NOT EXISTS saleItems (id INTEGER PRIMARY KEY AUTOINCREMENT, saleId INTEGER NOT NULL, productId INTEGER NOT NULL, quantity INTEGER NOT NULL, unitPrice TEXT NOT NULL, discount TEXT NOT NULL DEFAULT '0', location TEXT, costPrice TEXT NOT NULL DEFAULT '0');
  ...
  try { localDb.exec("ALTER TABLE saleItems ADD COLUMN discount TEXT NOT NULL DEFAULT '0'"); } catch {}
  try { localDb.exec("ALTER TABLE saleItems ADD COLUMN location TEXT"); } catch {}
  return localDb;
}
...
export async function listInstallments() { const db = getLocalDb(); return db.prepare(`SELECT i.*, s.invoiceNo, s.customerName, s.customerPhone, s.subtotal, s.paidAmount AS salePaidAmount, COUNT(ip.id) AS paymentCount, MAX(ip.createdAt) AS lastPaymentAt FROM installments i INNER JOIN sales s ON s.id=i.saleId LEFT JOIN installmentPayments ip ON ip.installmentId=i.id GROUP BY i.id ORDER BY i.dueDate DESC`).all().map((item: any) => ({ ...item, payments: db.prepare("SELECT id,amount,paymentMethod,note,createdAt FROM installmentPayments WHERE installmentId=? ORDER BY createdAt ASC").all(item.id) })); }
...
export async function listSales() { return getLocalDb().prepare(`SELECT s.*, COALESCE((SELECT u.name FROM users u WHERE u.employeeCode=s.sellerCode LIMIT 1), (SELECT u.name FROM users u WHERE u.id=s.sellerId LIMIT 1), 'موظف غير مسجل') AS sellerName, GROUP_CONCAT(DISTINCT COALESCE(si.location,p.location)) AS locations FROM sales s LEFT JOIN saleItems si ON si.saleId=s.id LEFT JOIN products p ON p.id=si.productId GROUP BY s.id ORDER BY s.createdAt DESC`).all(); }
export async function getSalesByEmployee(filters: { from?: Date; to?: Date } = {}) { ensureMonthEndExpenses(); const db = getLocalDb(); const conditions: string[] = []; const values: unknown[] = []; if (filters.from) { conditions.push("s.createdAt>=?"); values.push(filters.from.toISOString()); } if (filters.to) { conditions.push("s.createdAt<=?"); values.push(filters.to.toISOString()); } const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : ""; const rows = db.prepare(`SELECT COALESCE(s.sellerId, 0) AS sellerId, COALESCE(s.sellerCode, 'غير محدد') AS sellerCode, COALESCE((SELECT u2.name FROM users u2 WHERE u2.employeeCode=s.sellerCode LIMIT 1), u.name, 'موظف غير مسجل') AS sellerName, COUNT(s.id) AS invoiceCount, COALESCE(SUM(CAST(s.subtotal AS REAL)), 0) AS totalSales, COALESCE(SUM(CAST(s.paidAmount AS REAL)), 0) AS totalPaid, COALESCE(SUM(CAST(s.subtotal AS REAL) - CAST(s.paidAmount AS REAL)), 0) AS totalDue FROM sales s LEFT JOIN users u ON u.id=s.sellerId ${where} GROUP BY s.sellerId, s.sellerCode, u.name ORDER BY totalSales DESC`).all(...values) as any[]; return rows.map(row => ({ sellerId: Number(row.sellerId), sellerCode: String(row.sellerCode), sellerName: String(row.sellerName), invoiceCount: Number(row.invoiceCount), totalSales: Number(row.totalSales), totalPaid: Number(row.totalPaid), totalDue: Number(row.totalDue) })); }
export async function getSaleDetails(id: number) { const sale = getLocalDb().prepare("SELECT s.*, COALESCE((SELECT u.name FROM users u WHERE u.employeeCode=s.sellerCode LIMIT 1), (SELECT u.name FROM users u WHERE u.id=s.sellerId LIMIT 1), 'موظف غير مسجل') AS sellerName FROM sales s WHERE s.id = ? LIMIT 1").get(id); if (!sale) return null; const items = getLocalDb().prepare("SELECT si.productId,p.name,COALESCE(si.location,p.location) AS location,p.unit,si.quantity,si.unitPrice,COALESCE(si.discount,'0') AS discount FROM saleItems si INNER JOIN products p ON p.id=si.productId WHERE si.saleId=?").all(id); const payments = sale.paymentMethod === "installment" ? getLocalDb().prepare("SELECT ip.*, ROW_NUMBER() OVER (ORDER BY ip.createdAt ASC) AS paymentNumber FROM installmentPayments ip INNER JOIN installments i ON i.id=ip.installmentId WHERE i.saleId=? ORDER BY ip.createdAt ASC").all(id) : []; return { sale, items, payments }; }
...
export async function getDailySummary(filters: { from: Date; to: Date }) { ensureMonthEndExpenses(); const db = getLocalDb(); const from = filters.from.toISOString(); const to = filters.to.toISOString(); const sales = db.prepare("SELECT COALESCE(SUM(CAST(subtotal AS REAL)),0) value, COUNT(*) count FROM sales WHERE createdAt>=? AND createdAt<=?").get(from, to) as any; const returns = db.prepare("SELECT COALESCE(SUM(CAST(total AS REAL)),0) value, COUNT(*) count FROM purchases WHERE movementType='return' AND createdAt>=? AND createdAt<=?").get(from, to) as any; const salesQuantity = db.prepare("SELECT COALESCE(SUM(si.quantity),0) value FROM saleItems si INNER JOIN sales s ON s.id=si.saleId WHERE s.createdAt>=? AND s.createdAt<=?").get(from, to) as any; const returnsQuantity = db.prepare("SELECT COALESCE(SUM(pi.quantity),0) value FROM purchaseItems pi INNER JOIN purchases p ON p.id=pi.purchaseId WHERE p.movementType='return' AND p.createdAt>=? AND p.createdAt<=?").get(from, to) as any; return { sales: Number(sales.value), salesCount: Number(sales.count), salesQuantity: Number(salesQuantity.value), returns: Number(returns.value), returnsCount: Number(returns.count), returnsQuantity: Number(returnsQuantity.value) }; }
function ensureMonthEndExpenses() {
  const date = new Date();
  const lastDay = new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate();
  if (date.getDate() < Math.min(30, lastDay)) return;
  const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
  const db = getLocalDb();
  const employees = db.prepare("SELECT id,name,salary FROM users WHERE role <> 'admin' AND isActive=1 AND CAST(salary AS REAL)>0").all() as any[];
  for (const employee of employees) {
    const exists = db.prepare("SELECT id FROM expenses WHERE category='مرتبات' AND title=? AND createdAt LIKE ? LIMIT 1").get(`راتب ${employee.name}`, `${monthKey}%`);
    if (!exists) db.prepare("INSERT INTO expenses (title,category,amount,notes,createdAt) VALUES (?,?,?,?,?)").run(`راتب ${employee.name}`, "مرتبات", String(employee.salary), "خصم تلقائي في آخر يوم من الشهر", now());
  }
}

export async function getReportSummary(filters: { from?: Date; to?: Date; sellerId?: number; location?: string } = {}) { ensureMonthEndExpenses(); const db = getLocalDb(); const conditions: string[] = []; const values: unknown[] = []; if (filters.from) { conditions.push("s.createdAt>=?"); values.push(filters.from.toISOString()); } if (filters.to) { conditions.push("s.createdAt<=?"); values.push(filters.to.toISOString()); } if (filters.sellerId) { conditions.push("s.sellerId=?"); values.push(filters.sellerId); } if (filters.location) { conditions.push("COALESCE(si.location,p.location)=?"); values.push(filters.location); } const where = conditions.length ? ` WHERE ${conditions.join(" AND ")}` : ""; const sales = db.prepare(`SELECT COALESCE(SUM(CASE WHEN si.id IS NULL THEN CAST(s.subtotal AS REAL) ELSE CAST(si.unitPrice AS REAL) * si.quantity - CAST(COALESCE(si.discount, '0') AS REAL) END),0) AS value, COALESCE(SUM(CASE WHEN si.id IS NULL THEN 0 ELSE (CAST(si.unitPrice AS REAL) * si.quantity - CAST(COALESCE(si.discount, '0') AS REAL) - CAST(COALESCE(NULLIF(si.costPrice, '0'), p.costPrice, '0') AS REAL) * si.quantity) END),0) AS grossProfit FROM sales s LEFT JOIN saleItems si ON si.saleId=s.id LEFT JOIN products p ON p.id=si.productId${where}`).get(...values) as any; const expenseConditions: string[] = []; const expenseValues: unknown[] = []; if (filters.from) { expenseConditions.push("createdAt>=?"); expenseValues.push(filters.from.toISOString()); } if (filters.to) { expenseConditions.push("createdAt<=?"); expenseValues.push(filters.to.toISOString()); } const expenseQuery = `SELECT COALESCE(SUM(CAST(amount AS REAL)),0) value FROM expenses${expenseConditions.length ? ` WHERE ${expenseConditions.join(" AND ")}` : ""}`; const expenses = db.prepare(expenseQuery).get(...expenseValues) as any; const stock = db.prepare(`SELECT COALESCE(SUM(stockQty * CAST(costPrice AS REAL)),0) value FROM products WHERE isActive=1${filters.location ? " AND location=?" : ""}`).get(...(filters.location ? [filters.location] : [])) as any; const installments = db.prepare("SELECT COALESCE(SUM(CAST(amount AS REAL)-CAST(paidAmount AS REAL)),0) value, COUNT(*) count FROM installments WHERE status <> 'paid'").get() as any; const installmentSales = db.prepare(`SELECT COALESCE(SUM(CAST(s.subtotal AS REAL)),0) value, COALESCE(SUM(CAST(s.subtotal AS REAL)-CAST(s.paidAmount AS REAL)),0) due FROM sales s WHERE s.paymentMethod='installment'${filters.from ? " AND s.createdAt>=?" : ""}${filters.to ? " AND s.createdAt<=?" : ""}`).get(...[...(filters.from ? [filters.from.toISOString()] : []), ...(filters.to ? [filters.to.toISOString()] : [])]) as any; return { sales: Number(sales.value), expenses: Number(expenses.value), grossProfit: Number(sales.grossProfit), profit: Number(sales.grossProfit) - Number(expenses.value), stockValue: Number(stock.value), installmentsDue: Number(installments.value), installmentsCount: Number(installments.count), installmentSales: Number(installmentSales.value), installmentProfit: Number(installmentSales.value) - Number(expenses.value), installmentDue: Number(installmentSales.due) }; }
export async function createSale(input: { invoiceNo: string; customerId?: number; customerName?: string; customerPhone?: string; sellerId?: number; sellerCode?: string; paidAmount: string; paymentMethod: "cash" | "card" | "transfer" | "installment"; items: { productId: number; quantity: number; unitPrice: string; discount?: string; location?: string }[] }) {
  const total = input.items.reduce((sum, item) => sum + Math.max(0, Number(item.unitPrice) * item.quantity - Number(item.discount || 0)), 0);
  if (!Number.isFinite(total) || total <= 0) throw new Error("إجمالي الفاتورة غير صالح");
  if (input.paymentMethod === "installment" && (!input.customerName?.trim() || !input.customerPhone?.trim())) throw new Error("اسم العميل ورقم الهاتف إجباريان عند اختيار التقسيط");
  const paid = Math.max(0, Number(input.paidAmount) || 0);
  if (paid > total) throw new Error("المبلغ المدفوع أكبر من إجمالي الفاتورة");
  const status = paid >= total ? "paid" : paid > 0 ? "partial" : "unpaid";
  return transaction(db => {
    for (const item of input.items) { const product: any = db.prepare("SELECT * FROM products WHERE id=? AND isActive=1 LIMIT 1").get(item.productId); if (!product || product.stockQty < item.quantity) throw new Error("الكمية المطلوبة أكبر من الرصيد المتاح في المخزون"); }
    const timestamp = now();
    let customerId = input.customerId;
    if (input.paymentMethod === "installment" && !customerId) {
      const existing: any = db.prepare("SELECT id FROM customers WHERE phone=? LIMIT 1").get(input.customerPhone!.trim());
      customerId = existing?.id || Number(db.prepare("INSERT INTO customers (name,phone,address,outstandingBalance,createdAt) VALUES (?,?,NULL,?,?)").run(input.customerName!.trim(), input.customerPhone!.trim(), "0", timestamp).lastInsertRowid);
    }
    const seller: any = input.sellerId ? db.prepare("SELECT employeeCode FROM users WHERE id=? AND isActive=1 LIMIT 1").get(input.sellerId) : null;
    const effectiveSellerCode = seller?.employeeCode || input.sellerCode || null;
    const result = db.prepare("INSERT INTO sales (invoiceNo,customerId,customerName,customerPhone,sellerId,sellerCode,subtotal,paidAmount,paymentMethod,status,createdAt) VALUES (?,?,?,?,?,?,?,?,?,?,?)").run(input.invoiceNo, customerId || null, input.customerName?.trim() || null, input.customerPhone?.trim() || null, input.sellerId || null, effectiveSellerCode, total.toFixed(2), paid.toFixed(2), input.paymentMethod, status, timestamp);
    const saleId = Number(result.lastInsertRowid);
    for (const item of input.items) { const product: any = db.prepare("SELECT costPrice,location FROM products WHERE id=? LIMIT 1").get(item.productId); db.prepare("INSERT INTO saleItems (saleId,productId,quantity,unitPrice,discount,location,costPrice) VALUES (?,?,?,?,?,?,?)").run(saleId, item.productId, item.quantity, item.unitPrice, String(item.discount || "0"), item.location || product?.location || "المخزن", String(product?.costPrice ?? "0")); db.prepare("UPDATE products SET stockQty=stockQty-?,updatedAt=? WHERE id=?").run(item.quantity, timestamp, item.productId); db.prepare("INSERT INTO inventoryMovements (productId,type,quantity,referenceId,note,createdAt) VALUES (?,?,?,?,?,?)").run(item.productId, "sale", -item.quantity, saleId, `فاتورة بيع ${input.invoiceNo}`, timestamp); }
    if (input.paymentMethod === "installment" && customerId && total > paid) { const due = (total-paid).toFixed(2); db.prepare("INSERT INTO installments (saleId,customerId,dueDate,amount,paidAmount,status,createdAt) VALUES (?,?,?,?,?,'pending',?)").run(saleId, customerId, new Date(Date.now()+30*86400000).toISOString(), due, "0", timestamp); db.prepare("UPDATE customers SET outstandingBalance=CAST(outstandingBalance AS REAL)+? WHERE id=?").run(due, customerId); }
...
export async function recordInstallmentPayment(input: { installmentId: number; amount: string; paymentMethod: "cash" | "card" | "transfer"; note?: string }) { return transaction(db => { const installment: any = db.prepare("SELECT * FROM installments WHERE id=? LIMIT 1").get(input.installmentId); if (!installment) throw new Error("القسط غير موجود"); const amount = Number(input.amount); const remaining = Number(installment.amount) - Number(installment.paidAmount); if (!Number.isFinite(amount) || amount <= 0 || amount > remaining + 0.001) throw new Error("مبلغ السداد غير صالح"); const timestamp = now(); db.prepare("INSERT INTO installmentPayments (installmentId,amount,paymentMethod,note,createdAt) VALUES (?,?,?,?,?)").run(input.installmentId, amount.toFixed(2), input.paymentMethod, input.note || null, timestamp); const paid = Number(installment.paidAmount) + amount; const status = paid >= Number(installment.amount) - 0.001 ? "paid" : "partial"; db.prepare("UPDATE installments SET paidAmount=?,status=? WHERE id=?").run(Math.min(paid, Number(installment.amount)).toFixed(2), status, input.installmentId); db.prepare("UPDATE customers SET outstandingBalance=MAX(0,CAST(outstandingBalance AS REAL)-?) WHERE id=?").run(amount, installment.customerId); return { paidAmount: Math.min(paid, Number(installment.amount)).toFixed(2), remaining: Math.max(0, Number(installment.amount)-paid).toFixed(2), status, paidAt: timestamp }; }); }
