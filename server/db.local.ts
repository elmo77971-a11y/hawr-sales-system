...
function getLocalDb() {
  ...
  localDb.exec(`
    ...
    CREATE TABLE IF NOT EXISTS saleItems (id INTEGER PRIMARY KEY AUTOINCREMENT, saleId INTEGER NOT NULL, productId INTEGER NOT NULL, quantity INTEGER NOT NULL, unitPrice TEXT NOT NULL, costPrice TEXT NOT NULL DEFAULT '0');
...
export async function createEmployee(input: { name: string; email?: string; employeeCode: string; password?: string; role?: "user" | "admin" }) {
  if (input.role === "admin") throw new Error("لا يمكن إنشاء مدير إضافي من شاشة الموظفين المحلية");
  const timestamp = now(); const salt = createPasswordSalt(); const result = getLocalDb().prepare(`INSERT INTO users (openId,name,email,employeeCode,passwordHash,passwordSalt,role,isActive,loginMethod,createdAt,updatedAt,lastSignedIn) VALUES (?,?,?,?,?,?,?,1,'employee-password',?,?,?)`).run(`employee-${input.employeeCode}-${Date.now()}`, input.name, input.email || null, input.employeeCode, hashPassword(input.password || input.employeeCode, salt), salt, input.role || "user", timestamp, timestamp, timestamp);
...
export async function getSaleDetails(id: number) { const sale = getLocalDb().prepare("SELECT s.*, COALESCE((SELECT u.name FROM users u WHERE u.employeeCode=s.sellerCode LIMIT 1), (SELECT u.name FROM users u WHERE u.id=s.sellerId LIMIT 1), 'موظف غير مسجل') AS sellerName FROM sales s WHERE s.id = ? LIMIT 1").get(id); if (!sale) return null; const items = getLocalDb().prepare("SELECT si.productId,p.name,p.location,p.unit,si.quantity,si.unitPrice FROM saleItems si INNER JOIN products p ON p.id=si.productId WHERE si.saleId=?").all(id); const payments = sale.paymentMethod === "installment" ? getLocalDb().prepare("SELECT ip.* FROM installmentPayments ip INNER JOIN installments i ON i.id=ip.installmentId WHERE i.saleId=? ORDER BY ip.createdAt ASC").all(id) : []; return { sale, items, payments }; }
