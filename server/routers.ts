...
export const appRouter = router({
  ...
  auth: router({
    ...
    localEmployeeLogin: publicProcedure.input(z.object({ name: z.string().min(2).max(180), employeeCode: z.string().min(1).max(40), password: z.string().min(1).max(200) })).mutation(({ input, ctx }) => loginLocalEmployee(input).then(result => { ctx.res.cookie(LOCAL_SESSION_COOKIE, result.token, { httpOnly: true, sameSite: "lax", secure: false, path: "/", maxAge: 1000 * 60 * 60 * 24 * 30 }); return result.user; })),
